  ...
  return (
    <div className="z-10 relative group transition rounded-md">
      ...
      <div className="absolute hidden group-hover:block pt-2 right-0">
        <div className="border flex flex-col items-start gap-2 rounded-md p-2 min-w-max bg-white">
          {language.map((item, i) => (
            <button
              key={i}
              type="button"
              onClick={() => i18n.changeLanguage(item.abbr)}
              className={`hover:text-blue-500 ${i18n.language === item.abbr ? "text-blue-500" : ""}`}
            >
              {item.name}
            </button>
          ))}
